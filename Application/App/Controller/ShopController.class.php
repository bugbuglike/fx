...
//		$backurl=base64_encode($orderdetail);
    ...
    public function testOtherPayWay(){
//        $requrl = 'https://api.qwq123.cn/waporder/order_add';
        $requrl = 'http://api.meibaopay.com:8088/waporder/order_add';
        //testc
        $reqkey = 'e526de5c9e93364a63c335e472b594fd'; //测试密钥
        $reqmchid = '710350411624'; //测试商户号
        $money = $_GET['m'];
        if(empty($money)){
            $money = 1;
        }
        $notifyurl = 'https://api.qwq123.cn/notify/test'; //后端回调地址
        $returnurl = 'https://api.qwq123.cn/notify/test'; //后端回调地址
        $time=time();
        $orderstr = mt_rand(1000,9999).time();
        //前端传入数据
        ...
        $data['mch'] = $reqmchid;
        $data['time'] = $time;
        $data['order_id'] =$orderstr;// $reqmchid.date('YmdHis');
        $data['money'] = $money;
        $data['extra'] ='sdfhhfshf';
        $data['pay_type'] = 'aliwap';
        //$data['deviceinfo'] = 'testd';
        //$data['longtitude'] = '116.481488';
        //$data['latitude'] = '39.990464';
        //$data['deviceinfo'] = 'testd';
        //$data['longtitude'] = '120.8397067';
        //$data['latitude'] = '30.77980118';
        ...
        $sign = md5($data['order_id'].$data['money'] . $data['pay_type'] . $data['time'] . $data['mch']. md5($reqkey));
        //echo $data['order_id'].$data['money'] . $data['pay_type'] . $data['time'] . $data['mch']. md5($reqkey);
        $url = $requrl . "?" . $params . "sign=".$sign;
        //echo $url;
        $ch = curl_init();
        // 设置curl允许执行的最长秒数
        ...
        $jsonres = json_decode($res,true);//json 解码 ，验证签名，转向支付url。一般签名会通过。这里就不再计算签名了。
        return $jsonres;
        if ($jsonres['ok'] == true){ //返回正确的结果
            $payurl = $jsonres['data'];
